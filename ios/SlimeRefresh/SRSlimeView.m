...
static CGPoint kPoint;//他的作用很淡...疼
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _toPoint = _startPoint = CGPointMake(POINT_X,
                                             frame.size.height / 2);
        
        kPoint = CGPointMake(POINT_X,
                             frame.size.height / 2);//自写
                _viscous = 55.0f;
        _radius = POINT_LARGE;//这个值可以改变圆圈的大小
...
- (void)setStartPoint:(CGPoint)startPoint
{
    ...
    if (_state == SRSlimeStateNormal) {
        ...
        NSLog(@"设置了以后的开始位置是%@",NSStringFromCGPoint(_startPoint));
        [self setNeedsDisplay];
    }
}
