...
- (void)viewDidLoad
{
    ...
    UILabel *heightLabel = [[UILabel alloc] initWithFrame:CGRectMake(weightView.frame.origin.x + 36/2, 36/2, 80, 15)];
    heightLabel.textColor = [UIColor colorWithRed:94/255.0 green:101.0/255.0 blue:113/255.0 alpha:1.0];
    heightLabel.backgroundColor = [UIColor blueColor];
    heightLabel.textAlignment = NSTextAlignmentLeft;
    heightLabel.font = [UIFont systemFontOfSize:SECOND_TITLE_SIZE];
    heightLabel.text = @"身高:163cm";
    [weightConditionView addSubview:heightLabel];
    
    UILabel *previousWeightLabel = [[UILabel alloc] initWithFrame:CGRectMake(heightLabel.frame.origin.x, heightLabel.frame.origin.y + heightLabel.frame.size.height + 1, 170, 15)];
    previousWeightLabel.textColor = [UIColor colorWithRed:145/255.0 green:154/255.0 blue:170/255.0 alpha:1.0];
    previousWeightLabel.backgroundColor = [UIColor yellowColor];
    previousWeightLabel.textAlignment = NSTextAlignmentLeft;
    previousWeightLabel.font = [UIFont systemFontOfSize:SECOND_TITLE_SIZE];
    previousWeightLabel.text = @"孕前体重:50kg";
    [weightConditionView addSubview:previousWeightLabel];
    
    UILabel *recreaseLabel = [[UILabel alloc] initWithFrame:CGRectMake(heightLabel.frame.origin.x, previousWeightLabel.frame.origin.y + previousWeightLabel.frame.size.height + 1, 170, 15)];
    recreaseLabel.textColor = [UIColor colorWithRed:145/255.0 green:154/255.0 blue:170/255.0 alpha:1.0];
    recreaseLabel.backgroundColor = [UIColor redColor];
    recreaseLabel.textAlignment = NSTextAlignmentLeft;
    recreaseLabel.font = [UIFont systemFontOfSize:SECOND_TITLE_SIZE];
    recreaseLabel.text = @"目前比孕前体总增长:3kg";
    [weightConditionView addSubview:recreaseLabel];
...
- (void)modifyData:(UIButton *)btn
{
    if (self.actionSheetView == nil) {
        ...
        [_actionSheetView setTitlesForComponenets:[NSArray arrayWithObjects:
                                                   array1,
                                                   array2,
                                                   [NSArray arrayWithObjects:@"kg", nil],
                                                   nil]];
...
- (void)actionSheetPickerView:(BTSheetPickerview *)pickerView didSelectTitles:(NSArray*)titles
{
    ...
    NSLog(@"最新体重是%0.1f",[str floatValue]);
    NSLog(@"选择的宫高是%@",titles);
